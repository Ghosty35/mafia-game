...
import UsernamePrompt from '../dashboard/UsernamePrompt';
...
export default function HustlersWayPage() {
  ...
  useEffect(() => {
    ...
    supabase.rpc('get_hustler_tasks').then(({ data: d, error }) => {
      if (!active) return;
      if (error) {
        setLoadError(error.message);
      } else if (d) {
        setData(d as HustlerData);
        setLoadError(null);
      }
    });
    return () => {
  ...
  const claim = async (scope: 'daily' | 'weekly' | 'family', taskId?: string) => {
    ...
    if (error) {
      ...
      const famRes = res as { success?: boolean; family_power?: number } | null;
      if (famRes?.success) {
        setMsg(t('hw_family_claimed', { power: famRes.family_power ?? 0 }));
        await refreshPlayer();
    ...
    <main className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full space-y-4">
      {!player?.username && (
        <div className="text-center py-10">
          <p className="text-zinc-400 mb-4">{t('username_title')}</p>
          <UsernamePrompt onClaimed={async () => { await refreshPlayer(); retry(); }} />
        </div>
      )}

      {player?.username && loadError && (
        <div className="text-center py-10 space-y-4">
          <p className="text-red-400">{t('hw_claim_fail')}</p>
          <p className="text-xs text-zinc-500 break-all">{loadError}</p>
          <button onClick={retry} className="px-4 py-2 bg-red-700 hover:bg-red-600 rounded-lg text-sm font-semibold transition-colors">
            {t('common_error')}
          </button>
        </div>
      )}

      {player?.username && !loadError && !data && (
