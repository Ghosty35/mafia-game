-- 032: Further fix for "column reference "crime_key" is ambiguous"
-- The previous alias fixed the SELECT, but the INSERT ... ON CONFLICT (..., crime_key)
-- still causes ambiguity because the function parameter "crime_key" is in scope.
-- Solution: Use ON CONFLICT ON CONSTRAINT instead of column list.

-- Make sure the primary key constraint has a predictable name
ALTER TABLE public.crime_cooldowns 
  DROP CONSTRAINT IF EXISTS crime_cooldowns_pkey;

ALTER TABLE public.crime_cooldowns 
  ADD CONSTRAINT crime_cooldowns_pkey PRIMARY KEY (player_id, crime_key);
...
BEGIN
  ...
  -- Use constraint name instead of bare column "crime_key" to avoid ambiguity with function param
  INSERT INTO public.crime_cooldowns (player_id, crime_key, available_at)
  VALUES (p.id, c.key, next_available)
  ON CONFLICT ON CONSTRAINT crime_cooldowns_pkey 
...
COMMENT ON FUNCTION public.commit_crime(text) IS 'Use ON CONFLICT ON CONSTRAINT to avoid ambiguous crime_key column reference with function parameter.';
