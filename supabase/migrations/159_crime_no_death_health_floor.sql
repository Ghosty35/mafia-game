-- 159_crime_no_death_health_floor.sql
-- Player can only die from murder, not crimes.
-- Crimes now require minimum 25% health. Below that, server returns HEALTH_TOO_LOW.
...
BEGIN
  ...
  -- No death from crimes: only murder can kill.
  -- Minimum health guard: must have at least 25% health to commit a crime.
  ...
  -- No death from crimes: floor at 1% health instead of 0
